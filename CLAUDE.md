...
- `bun run tauri dev` on Linux runs the webview from `http://localhost:1420`, which the prod API Gateway CORS allowlist rejects (only `tauri.localhost`, by audit decision in `berzerk-infra`). `.env.development` + the `server.proxy` in `vite.config.ts` route `/api` and `/ws` through Vite (same-origin) — don't point `VITE_SEPARACAO_*` at the API directly in dev. Change the targets via `DEV_PROXY_*_TARGET`; env changes need a `tauri dev` restart.
